...
async def basic_memory_example():
	"""
	Demonstrates basic memory integration with Gemini SDK.
	...
	async with memory(agent="gemini_assistant", client=letta_client):
		# First turn - introduce yourself
		print("You: My name is Alice and I work on AI agents.")
		response = await model.generate_content_async("My name is Alice and I work on AI agents.")
		print(f"Gemini: {response.text}\n")

		# Second turn - ask a question
		print("You: What are some interesting AI projects I could work on?")
		response = await model.generate_content_async("What are some interesting AI projects I could work on?")
		print(f"Gemini: {response.text}\n")

	print("✅ Conversation saved to Letta! Agent 'gemini_assistant' will remember this.")
	print()
...
async def cross_session_memory_example():
	...
	This example shows how Gemini remembers information from previous sessions
	by using the same agent name. Run this after basic_memory_example() to see
	Gemini recall information from the previous conversation.
	"""
	print("=" * 80)
	...
	async with memory(agent="gemini_assistant", client=letta_client):
		# Ask Gemini to recall information from previous session
		print("You: What is my name and what do I work on?")
		response = await model.generate_content_async("What is my name and what do I work on?")
		print(f"Gemini: {response.text}\n")

	print("✅ Gemini remembered information from the previous session!")
	print()
...
async def streaming_example():
	...
	async with memory(agent="gemini_assistant", client=letta_client):
		...
		response = await model.generate_content_async("Count to 5 for me.", stream=True)
...
async def capture_only_example():
	...
	Useful for logging conversations without influencing Gemini's responses.
	"""
	print("=" * 80)
	...
	async with memory(agent="capture_only_agent", client=letta_client, capture_only=True):
		...
		response = await model.generate_content_async("Hello! I'm testing capture-only mode.")
		print(f"Gemini: {response.text}\n")
...
async def main():
	"""
	Run all examples demonstrating memory integration with Gemini SDK.
	...
	print("\n")
	print("╔" + "=" * 78 + "╗")
	print("║" + " " * 20 + "Letta Memory Integration with Gemini SDK" + " " * 19 + "║")
	print("╚" + "=" * 78 + "╝")
	print()
	print("These examples demonstrate how to use Letta memory with Gemini SDK.")
